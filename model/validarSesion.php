<?php
    class validarSesion{
        ...
        public function iniciarSesionCli($emailC, $claveC){
            ...
            $sql = "SELECT * FROM clientes WHERE Correo_cliente = :emailC";
            $result = $conexion->prepare($sql);
            $result->bindParam(":emailC", $emailC);
            $result->execute();

            // fetch se usa para convertir en arreglo 
            if ($f = $result->fetch()) {
                // comparamos las claves
                if ($claveC == $f['Clave_cliente']) {
                    // comparamos el estado
                    if ($f['ID_estado'] == 1) {

                        //variables de inicio de sesion
                        session_start();
                        $_SESSION['id'] = $f['ID_cliente'];
                        $_SESION['autenticacion'] = "SI";
                        
                        if ($f['ID_rol'] == 3) {
                            echo '<script> alert("Bienvenido cliente") </script>';
                            echo "<script> location.href='../view/cliente' </script>"; 
                        }
                ...
                if ($claveE == $f['Clave_empleado']) {
                    ...
                    if ($f['ID_estado'] == 1) {
                        ...
                        if ($f['ID_rol'] == 2) {
                            ...
                            echo "<script> location.href='../view/vendedor' </script>"; 
                        }
